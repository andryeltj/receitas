...
-o APK::Cache-Limit=125829120
-o Dir::Etc::sourcelist=./sources.list
-o Dir::State=./tmp
-o Dir::Cache=./tmp
-o Dir::State::status=./status
-o Dir::Etc::sourceparts=-
-o APK::Get::List-Cleanup=0
-o APK::Get::AllowUnauthenticated=1
-o Debug::pkgProblemResolver=true
-o Debug::pkgDepCache::AutoInstall=true
-o APK::Add-Recommends=0
-o APK::Add-Suggests=0
"
...
generate_appimage()
{
  ...
  echo $ARCH
  ARCH=$(uname -i) ./AppImageAssistant ./$APP.AppDir/ ../out/$APP"-"$VERSION".glibc"$GLIBC_NEEDED"-"$ARCH".AppImage"
}
...
get_icon()
{
  ...
  find ./usr/share/icons -path *scalable* -name $LOWERAPP.png -exec cp {} . \; 2>/dev/null || true
  ...
  find ./usr/share/icons -path *scalable* -name $LOWERAPP.svg -exec cp {} . \; 2>/dev/null || true
...
get_version()
{
  THEAPK=$(find ../*.apk -name $LOWERAPP"_*" | head -n 1)
  if [ -z "$THEAPK" ] ; then
    echo "Version could not be determined from the .apk; you need to determine it manually"
  fi
  VERSION=$(echo $THEAPK | cut -d "~" -f 1 | cut -d "_" -f 2 | cut -d "-" -f 1 | sed -e 's|1%3a||g' | sed -e 's|.dfsg||g' )
